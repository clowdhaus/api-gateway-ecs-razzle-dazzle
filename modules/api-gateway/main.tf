...
resource "aws_apigatewayv2_stage" "this" {
  ...
  dynamic "access_log_settings" {
    ...
    content {
      destination_arn = try(aws_cloudwatch_log_group.stage[0].arn, access_log_settings.value.destination_arn)
      format          = try(jsonencode(access_log_settings.value.format), access_log_settings.value.format)
    }
  }
  ...
  dynamic "default_route_settings" {
    ...
    content {
      ...
      throttling_burst_limit   = try(default_route_settings.value.throttling_burst_limit, 300)
  ...
  dynamic "route_settings" {
    ...
    content {
      ...
      throttling_rate_limit    = try(route_settings.value.throttling_rate_limit, 100)
    }
  }
...
resource "aws_cloudwatch_log_group" "stage" {
  count = var.create && var.create_stage && var.create_cloudwatch_log_group ? 1 : 0

  name              = "/aws/api-gateway/${var.name}/${replace(var.stage_name, "$", "")}"
...
resource "aws_apigatewayv2_vpc_link" "this" {
  ...
  name               = try(each.value.name, each.key)
  security_group_ids = local.create_security_group ? try([aws_security_group.this[0].id], []) : try(each.value.security_group_ids, [])
